...
class WSConnection implements \CharlotteDunois\Events\EventEmitterInterface
{
     ...
     */
    public function __construct(\CharlotteDunois\Yasmin\WebSocket\WSManager $wsmanager, int $shardID, string $compression)
    {
    ...
    public function connect(bool $reconnect = false)
    {
        ...
        $connector($this->wsmanager->gateway)->done(
            function (\Ratchet\Client\WebSocket $conn) use (&$ready, $deferred, $reconnect) {
