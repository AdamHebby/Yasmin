...
class VoiceStateUpdate implements \CharlotteDunois\Yasmin\Interfaces\WSEventInterface
{
    ...
    public function handle(\CharlotteDunois\Yasmin\WebSocket\WSConnection $ws, $data): void
    {
        $this->client->fetchUser($data['user_id'])->done(
            function (\CharlotteDunois\Yasmin\Models\User $user) use ($data) {
                if (empty($data['channel_id'])) {
                    ...
                    if ($guild) {
                        ...
                        $member->done(
                            function (\CharlotteDunois\Yasmin\Models\GuildMember $member) use ($data) {
                                $oldMember = null;
                                if ($this->clones) {
                    ...
                    if ($channel instanceof \CharlotteDunois\Yasmin\Models\VoiceChannel) {
                        ...
                        $member->done(
                            function (\CharlotteDunois\Yasmin\Models\GuildMember $member) use ($data, $channel) {
                                $oldMember = null;
                                if ($this->clones) {
                                    $oldMember = clone $member;
                                }
