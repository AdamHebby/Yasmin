...
class MessageReactionRemove implements \CharlotteDunois\Yasmin\Interfaces\WSEventInterface
{
    ...
    public function handle(\CharlotteDunois\Yasmin\WebSocket\WSConnection $ws, $data): void
    {
        ...
        if ($channel instanceof \CharlotteDunois\Yasmin\Interfaces\TextChannelInterface) {
            ...
            $message->done(
                function (\CharlotteDunois\Yasmin\Models\Message $message) use ($data, $channel, $id, $reaction) {
                    if (!$reaction) {
                        $reaction = $message->reactions->get($id);
                        if (!$reaction) {
                    ...
                    $this->client->fetchUser($data['user_id'])->done(
                        function (\CharlotteDunois\Yasmin\Models\User $user) use ($id, $message, $reaction) {
