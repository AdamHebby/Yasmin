...
trait TextChannelTrait
{
            ...
            function (callable $resolve, callable $reject) use ($messages, $reason, $filterOldMessages) {
                ...
                $messages->done(
                    function ($messages) use ($reason, $filterOldMessages, $resolve, $reject) {
                        ...
                        $this->client->apimanager()->endpoints->channel->bulkDeleteMessages($this->id, $messages, $reason)->done(
            ...
            function (callable $resolve, callable $reject) use ($content, $options) {
                \CharlotteDunois\Yasmin\Utils\MessageHelpers::resolveMessageOptionsFiles($options)->done(
                    function ($files) use ($content, $options, $resolve, $reject) {
                        ...
                        if (isset($options['split'])) {
                            ...
                            if (\count($messages) > 1) {
                                ...
                                $chunkedSend = function ($msg, $files = null) use ($collection, $reject) {
                                    return $this->client->apimanager()->endpoints->channel->createMessage($this->id, $msg, ($files ?? array()))->then(
                        ...
                        $this->client->apimanager()->endpoints->channel->createMessage($this->id, $msg, ($files ?? array()))->done(
     ...
     * @return int
     */
    public function isTypingSince(\CharlotteDunois\Yasmin\Models\User $user)
    {
