...
class APIRequest
{
     ...
     * @param array                                   $options
     */
    public function __construct(\CharlotteDunois\Yasmin\HTTP\APIManager $api, string $method, string $endpoint, array $options)
    {
        $this->api = $api;
     ...
     */
    public function execute(?\CharlotteDunois\Yasmin\Interfaces\RatelimitBucketInterface $ratelimit = null)
    {
...
*/
        return \CharlotteDunois\Yasmin\Utils\URLHelpers::makeRequest($request, $request->requestOptions)
            ->then(
                function (?\Psr\Http\Message\ResponseInterface $response) use ($ratelimit) {
                    if (!$response) {
                        return -1;
                    }
     ...
     */
    public static function decodeBody(\Psr\Http\Message\ResponseInterface $response)
    {
