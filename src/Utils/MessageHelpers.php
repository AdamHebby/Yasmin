...
class MessageHelpers
{
     ...
     * @return string
     */
    public static function cleanContent(\CharlotteDunois\Yasmin\Models\Message $message, string $text)
    {
        /**
...
*/
     ...
     */
    public static function parseMentions(?\CharlotteDunois\Yasmin\Client $client, string $content)
    {
                    ...
                    if ($val === '@everyone') {
                        ...
                        if ($client) {
                            $promises[] = $client->fetchUser($matches[2][$key])->then(
                                function (\CharlotteDunois\Yasmin\Models\User $user) use (&$bucket, $key, $type) {
        ...
        foreach ($options['files'] as $file) {
            ...
            if (\is_string($file)) {
                if (\filter_var($file, \FILTER_VALIDATE_URL)) {
                    $promises[] = \CharlotteDunois\Yasmin\Utils\URLHelpers::resolveURLToData($file)->then(
                        function ($data) use ($file) {
                            return array('name' => \basename($file), 'data' => $data);
                        }
                    );
                } elseif (\realpath($file)) {
                    $promises[] = \CharlotteDunois\Yasmin\Utils\FileHelpers::resolveFileResolvable($file)->then(
                        function ($data) use ($file) {
                            return array('name' => \basename($file), 'data' => $data);
                        }
                    );
            ...
            if (isset($file['path'])) {
                if (filter_var($file['path'], \FILTER_VALIDATE_URL)) {
                    $promises[] = \CharlotteDunois\Yasmin\Utils\URLHelpers::resolveURLToData($file['path'])->then(
                        function ($data) use ($file) {
                            $file['data'] = $data;
                            return $file;
                        }
                    ...
                    $promises[] = \CharlotteDunois\Yasmin\Utils\FileHelpers::getFilesystem()->getContents($file['path'])->then(
                        function ($data) use ($file) {
                            $file['data'] = $data;
                            return $file;
                        }
     ...
     */
    public static function splitMessage(string $text, array $options = array())
    {
