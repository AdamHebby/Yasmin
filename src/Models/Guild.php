...
class Guild extends ClientBase
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, array $guild, ?int $shardID = null)
    {
     ...
     */
    public function addMember($user, string $accessToken, array $options = array())
    {
     ...
     */
    public function createEmoji(string $file, string $name, $roles = array(), string $reason = '')
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($file, $name, $roles, $reason) {
                \CharlotteDunois\Yasmin\Utils\FileHelpers::resolveFileResolvable($file)->done(
                    function ($file) use ($name, $roles, $reason, $resolve, $reject) {
                        ...
                        $this->client->apimanager()->endpoints->emoji->createGuildEmoji($this->id, $options, $reason)->done(
                            function ($data) use ($resolve) {
     ...
     */
    public function fetchAuditLog(array $options = array())
    {
        return (new \React\Promise\Promise(
    ...
    public function pruneMembers(int $days, bool $withCount = false, string $reason = '')
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($days, $withCount, $reason) {
                $this->client->apimanager()->endpoints->guild->beginGuildPrune($this->id, $days, $withCount, $reason)->done(
                    function ($data) use ($resolve) {
    ...
    public function setChannelPositions(array $channels, string $reason = '')
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($channels, $reason) {
                ...
                $this->client->apimanager()->endpoints->guild->modifyGuildChannelPositions($this->id, $options, $reason)->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
    ...
    public function setRolePositions(array $roles, string $reason = '')
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($roles, $reason) {
                ...
                $this->client->apimanager()->endpoints->guild->modifyGuildRolePositions($this->id, $options, $reason)->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
