...
class Emoji extends ClientBase
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, ?\CharlotteDunois\Yasmin\Models\Guild $guild, array $emoji)
    {
    ...
    public function edit(array $options, string $reason = '')
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($options, $reason) {
                ...
                $this->client->apimanager()->endpoints->emoji->modifyGuildEmoji($this->guild->id, $this->id, $options, $reason)->done(
    ...
    public function delete(string $reason = '')
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($reason) {
                $this->client->apimanager()->endpoints->emoji->deleteGuildEmoji($this->guild->id, $this->id, $reason)->done(
                    function () use ($resolve) {
                        $resolve();
                    },
                    $reject
