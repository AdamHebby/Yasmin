...
class ClientUser extends User
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, $user)
    {
    ...
    {
        if ($avatar === null) {
            return $this->client->apimanager()->endpoints->user->modifyCurrentUser(array('avatar' => null))->then(
                function () {
                    return $this;
                }
            );
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($avatar) {
                \CharlotteDunois\Yasmin\Utils\FileHelpers::resolveFileResolvable($avatar)->done(
                    function ($data) use ($resolve, $reject) {
                        $image = \CharlotteDunois\Yasmin\Utils\DataHelpers::makeBase64URI($data);

                        $this->client->apimanager()->endpoints->user->modifyCurrentUser(array('avatar' => $image))->done(
    ...
    public function setPresence(array $presence, ?int $shardID = null)
    {
        ...
        if ($shardID === null) {
            ...
            return \React\Promise\all($prms)->then(
                function () {
                    return $this;
                }
            );
    ...
    public function setUsername(string $username)
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($username) {
                $this->client->apimanager()->endpoints->user->modifyCurrentUser(array('username' => $username))->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
     ...
     */
    public function createGroupDM(array $userWithAccessTokens, array $nicks = array())
    {
        return (new \React\Promise\Promise(
