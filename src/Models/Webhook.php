...
class Webhook extends ClientBase
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, array $webhook)
    {
    ...
    public function edit(array $options, string $reason = '')
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($data, $options, $reason) {
                \CharlotteDunois\Yasmin\Utils\FileHelpers::resolveFileResolvable(($options['avatar'] ?? ''))->done(
    ...
    public function send(string $content, array $options = array())
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($content, $options) {
                \CharlotteDunois\Yasmin\Utils\MessageHelpers::resolveMessageOptionsFiles($options)->done(
                    function ($files) use ($content, $options, $resolve, $reject) {
                        ...
                        if (!empty($options['split'])) {
                            ...
                            if (\count($messages) > 0) {
                                ...
                                $chunkedSend = function ($msg, $files = null) use ($collection, $reject) {
                                    return $this->executeWebhook($msg, ($files ?? array()))->then(
                        ...
                        $this->executeWebhook($msg, ($files ?? array()))->done(
                            function ($data) use ($resolve) {
                                $resolve($data);
                            },
                            $reject
    ...
    {
        return $this->client->apimanager()->endpoints->webhook->executeWebhook($this->id, $this->token, $opts, $files, array('wait' => true))->then(
            function ($data) {
