...
class GroupDMChannel extends DMChannel implements \CharlotteDunois\Yasmin\Interfaces\GroupDMChannelInterface
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, array $channel)
    {
    ...
    public function addRecipient($user, string $accessToken, string $nick = '')
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($user, $accessToken, $nick) {
                $this->client->apimanager()->endpoints->channel->groupDMAddRecipient($this->id, $user, $accessToken, $nick)->done(
