...
class PermissionOverwrite extends ClientBase
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, \CharlotteDunois\Yasmin\Interfaces\GuildChannelInterface $channel, array $permission)
    {
    ...
    public function edit($allow, $deny = null, string $reason = '')
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($options, $reason) {
                $this->client->apimanager()->endpoints->channel->editChannelPermissions($this->channel->getId(), $this->id, $options, $reason)->done(
    ...
    public function delete(string $reason = '')
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($reason) {
                $this->client->apimanager()->endpoints->channel->deleteChannelPermission($this->channel->getId(), $this->id, $reason)->then(
