...
class Message extends ClientBase
{
     ...
     * @internal
     */
    public function __construct(\CharlotteDunois\Yasmin\Client $client, \CharlotteDunois\Yasmin\Interfaces\TextChannelInterface $channel, array $message)
    {
    ...
    public function clearReactions()
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) {
                $this->client->apimanager()->endpoints->channel->deleteMessageReactions($this->channel->getId(), $this->id)->done(
     ...
     */
    public function collectReactions(callable $filter, array $options = array())
    {
     ...
     */
    public function edit(?string $content, array $options = array())
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($content, $options) {
                ...
                $this->client->apimanager()->endpoints->channel->editMessage($this->channel->getId(), $this->id, $msg)->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($timeout, $reason) {
                if ($timeout > 0) {
                    ...
                    $this->client->apimanager()->endpoints->channel->deleteMessage($this->channel->getId(), $this->id, $reason)->done(
                        function () use ($resolve) {
                            $resolve();
                        },
                        $reject
    ...
    public function pin()
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) {
                $this->client->apimanager()->endpoints->channel->pinChannelMessage($this->channel->getId(), $this->id)->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
    ...
    public function react($emoji)
    {
        ...
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) use ($emoji) {
                ...
                $prom = \CharlotteDunois\Yasmin\Utils\EventHelpers::waitForEvent($this->client, 'messageReactionAdd', $filter, array('time' => 30))->then(
                ...
                $this->client->apimanager()->endpoints->channel->createMessageReaction($this->channel->getId(), $this->id, $emoji)->done(
                    null,
     ...
     * @see    \CharlotteDunois\Yasmin\Traits\TextChannelTrait::send()
     */
    public function reply(string $content, array $options = array())
    {
    ...
    public function unpin()
    {
        return (new \React\Promise\Promise(
            function (callable $resolve, callable $reject) {
                $this->client->apimanager()->endpoints->channel->unpinChannelMessage($this->channel->getId(), $this->id)->done(
                    function () use ($resolve) {
                        $resolve($this);
                    },
                    $reject
    ...
    public function _addReaction(array $data)
    {
        ...
        if (!$reaction) {
            ...
            $reaction = new \CharlotteDunois\Yasmin\Models\MessageReaction(
                $this->client,
                $this,
                $emoji,
                array(
